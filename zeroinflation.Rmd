...
The example above generates values from a Poisson distribution with mean `r mean.poisson`. Although it has no zero-inflation, `r sprintf("%.1f%%", 100 * mean(dataset$Poisson == 0))` of the values are zero.
...
The second example generates a data from a zero-inflated Poisson distribution with mean `r mean.zeroinfl` and `r sprintf("%.0f, %%", 100 * prop.zeroinfl)` excest zero's. The actual proportion of zero's is `r sprintf("%.1f%%", 100 * mean(dataset$ZeroInflatedPoisson == 0))`.
...
For this example we generate a new dataset with two levels of Mean: `r sprintf("%.2f", c(mean.poisson, mean.zeroinfl))`. We generate both a Poisson and a zero-inflated Poisson variable. The latter has `r sprintf("%.0f, %%", 100 * prop.zeroinfl)` excess zero's. The proportion of zero's the variables are respectively `r sprintf("%.1f%%", 100 * mean(dataset$Poisson == 0))` and `r sprintf("%.1f%%", 100 * mean(dataset$ZeroInflatedPoisson == 0))`.
...
When we fit a proper zero-inflated model to the zero-inflated Poisson variable, then `r sprintf("%.1f%%", 100 * mean(simulated$ZeroInflatedZI > mean(dataset$ZeroInflatedPoisson == 0)))` of the simulated datasets have more zero's than the observed dataset. So the zero-inflation is proper handled.
